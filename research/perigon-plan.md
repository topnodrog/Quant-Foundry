...
**Connectivity:** `api.perigon.io` is reachable normally (verified — HTTP 404 on the
root path under the standard sandbox). An earlier one-off `ConnectionReset` was a
local internet drop, not an egress block; no special handling is needed.
